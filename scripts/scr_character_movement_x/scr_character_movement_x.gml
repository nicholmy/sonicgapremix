/// @description scr_character_movement_x();
// Handle a part of the X/Horizontal Movement.
   ...
   // Only continue if we're in one of the following states:
      if((Action == ActionNormal || Action = ActionSkid || (Action = ActionJump && JumpLock == false) || Action = ActionFly || Action = ActionFlydrop || Action = ActionSpring || Action = ActionGlidedrop || Action = ActionFan) && (Action != ActionGlide && Action != ActionClimb && Action != ActionSlide)){
