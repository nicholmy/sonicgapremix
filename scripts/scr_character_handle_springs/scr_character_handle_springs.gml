/// @description  scr_character_handle_springs();
//  Script to handle one of the main features in Sonic games. Springs!
    ...
    // Check if we collide with a spring:       
       if (SpringDiagonal != noone){
		...
		Flying_Spring = scr_instance_nth_nearest(x, y, obj_flying_spring, 1);
		
		if (Flying_Spring != noone) {
			if (scr_character_collision_bottom_object(x, y, Angle, spr_mask_large, Flying_Spring) && YSpeed >= 0) {               
                scr_character_angle(global.GravityAngle);
                Ground = false;
                if(HomingUsed == true){
                    HomingUsed  = false;
                }
                YSpeed = -Flying_Spring.strength;
				if (Action == ActionHammerDrop) {
					YSpeed = -Flying_Spring.superStrength;
					Flying_Spring.superBounce = true;
				} else {
					Flying_Spring.startBounce = true;
				}
