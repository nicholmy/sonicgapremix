/// @description  scr_character_handle_bosses()
//  Script to handle each boss.

	/*if(instance_exists(obj_boss_camera)){
       BossCameraTarget   = scr_character_collision_object(x, y, obj_boss_camera);
	   obj_camera.Cam_Subject = BossCameraTarget;
	   
	   //instance_create(BossCameraTarget.x + 96, BossCameraTarget.y, obj_npc_sth3_firebreather);
	} */
    ...
    // Hit/Destroy Breather:
       if(FireBreather != noone && (Action != ActionHurt && Action != ActionDie)){
          ...
          // Bounce:
             if(Action = ActionJump || Action = ActionRolling || (Action = ActionFly && YSpeed < 0) || Action = ActionGlide || Action = ActionHammerDrop){
    ...
    // Hurt Knuckles:   
       if((Action = ActionJump || Action = ActionHammerDrop) && (Knux[1] != noone || Knux[4] != noone) && (Knux[2] == noone && Knux[3] == noone)){
...
// Set Boss handle:
    if(instance_exists(obj_boss_generic)){
    
       Boss   = scr_character_collision_object(x, y, obj_boss_generic);

    // Hit/Destroy Boss:
       if(Boss != noone && (Action != ActionHurt && Action != ActionDie)){
          if(Boss.State != "Defeated" && Boss.State != "Hit"){
          ...
          // Bounce:
             ...
             AngleToBoss = degtorad(round(scr_character_wrap_angle(point_direction(x, y, Boss.x, Boss.y)-90)/22.5)*24);        
             if(Underwater == false){
                XSpeed        = sin(scr_character_wrap_angle(AngleToBoss-global.GravityAngle))*4;
                YSpeed        = cos(scr_character_wrap_angle(AngleToBoss-global.GravityAngle))*3;  
             }else{
             ...
             with(Boss){
                  if(HP != 0 && State == "Normal"){
                     ...
                     PlaySound(snd_object_destroy, global.SFXVolume, 1, 1);       
                      ...
                      scr_character_action_hit(Boss, false);             
             }   
